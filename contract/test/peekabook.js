...
contract('PeekABook', (accounts) => {
  it('test `advertise` and `invalidate`', async () => {
    ...
    await peekABookInstance.advertise(pairName, buyOrSell, amount, peerID, { from: owner });
    ...
    assert.equal(ad0.peerID, peerID, '`advertisement.peerID` is wrong');
    assert.equal(ad0.isValid, true, '`advertisement.isValid` is wrong');

    // Test: `advertise` should add another advertisement successfully, and the previously added
    //  ones should not be changed.
    await peekABookInstance.advertise('BTCUSDT', false, 100, peerID, { from: accounts[1] });
    ...
    assert.equal(ad1.isValid, true, '`advertisement.isValid` is wrong');
    ...
    assert.equal(ad0.isValid, ad0Again.isValid, '`advertisement.isValid` is wrong');
